...
tmux new-session -d -s rove-demo \
  "cd \"$ROOT\" && unset OM_API_KEY && uv run src/run.py rove_useful_patrol --log-level INFO >> /tmp/rove-demo.log 2>&1"
...
tmux new-session -d -s rove-console \
  "cd \"$ROOT\" && python3 -m http.server 8010 >/tmp/rove-console.log 2>&1"
