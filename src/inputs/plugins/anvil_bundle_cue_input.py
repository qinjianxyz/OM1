...
class AnvilBundleCueConfig(SensorConfig):
    """Configuration for scripted hackathon review cues."""

    cue_file: str = Field(default="hackathon/demo_cues.txt")
    input_name: str = Field(default="Judge Question")
    interval_seconds: float = Field(default=2.0)
    repeat: bool = Field(default=False)


class AnvilBundleCueInput(FuserInput[AnvilBundleCueConfig, Optional[str]]):
    """Emit a small scripted question stream for the no-microphone demo path."""

    def __init__(self, config: AnvilBundleCueConfig):
